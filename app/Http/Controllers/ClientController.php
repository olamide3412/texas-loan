...
class ClientController extends Controller
{
    ...
    public function update(Request $request, Client $client)
    {
        ...
        $validatedData = $request->validate([
            ...
            'first_name' => ['required', new OneWord],
            'other_name' => ['nullable', new OneWord],
            'email' => ['nullable', 'email', Rule::unique('clients', 'email')->ignore($client->id), Rule::unique('staff', 'email')],
            'date_of_birth' => ['required', 'date'],
            'gender' => ['required'],
            'phone_number' => ['required', 'min:11', Rule::unique('clients', 'phone_number')->ignore($client->id), Rule::unique('staff', 'phone_number')],
            'residential_address' => ['required'],
            'local_government' => ['required'],
            'state' => ['required'],
            'occupation' => ['required'],
